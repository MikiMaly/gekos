...
import { ArrowLeft, Sparkles, Check, Stethoscope, StickyNote, Trash2 } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="max-w-3xl mx-auto p-6">
        ...
        <section className="mb-8">
          <h2 className="text-xl font-semibold inline-flex items-center gap-2 mb-3">
            <StickyNote className="w-5 h-5 text-amber-500" /> Poznámky
          </h2>
          {notes.length === 0 ? (
            ...
            <ul className="space-y-1.5">
              {notes.map((n) => (
                <li
                  key={n.id}
                  className="flex items-start justify-between gap-3 py-2 px-3 rounded-md border border-border"
                >
                  <div className="flex-1 min-w-0">
                    <div className="text-xs text-muted-foreground mb-0.5">
                      {formatDateWithWeekday(n.date_prague)}
                    </div>
